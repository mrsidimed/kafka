...
const removeListener = consumer.on(DISCONNECT, e => {
    ...
    logException(`------------------DISCONNECT at ${e.timestamp}`);
    runConsumer();
});


consumer.on(DISCONNECT, async (e) => {
    console.error(`Disconnected at ${e.timestamp}, attempting to reconnect...`);
...
// function executeThreeQueries(query1, query2, query3, numero, callback) {  // to uncomment later
//     console.log("inside executeThreeQueries");
