                ...
                if (err) {
                    ...
                    var queryOrdre = `
                        SELECT 
                            ...
                            ordres.nni,
                            ordres.TYPEDOC,
                            ordres.PRENOM_FR as prenomFr,
                            ordres.PRENOM_AR as prenomAr,
                            ordres.NOM_FAMILLE_FR as nomFamilleFr,
                            ordres.NOM_FAMILLE_AR as nomFamilleAr,
                            ordres.DATE_NAISSANCE as dateNaissance,
                            ordres.LIEU_NAISSANCE_FR as lieuNaissanceFr,
                            ordres.LIEU_NAISSANCE_AR as lieuNaissanceAr,
                            ordres.MONTANT as montant,
                            ordres.TRANSPORT as transport,
                            typedemande.libelle as typeDemande,
                            typedocument.libelle as typeDocument,
                            cac.nom_cac as cacFr,
                            cac.nom_cac as cacAr,
                            cac.nom_cacar as cacAr
                        FROM ordres
                        INNER JOIN typedemande ON typedemande.code = ordres.TYPEDEM
                        INNER JOIN typedocument ON typedocument.code = ordres.TYPEDOC
                        INNER JOIN cac ON ordres.codecac = cac.cac
                        WHERE sent = 0
                        AND ${typedocSqlPart}
                        AND (ordres.numero NOT LIKE '00%' OR ordres.TYPEDOC = '5')
                        LIMIT ${kafkaParams["numberOfOrders"]}
                    `;
